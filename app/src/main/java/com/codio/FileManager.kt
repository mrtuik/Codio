...
class FileManager(context: Context) {
    val codioRoot = File(context.filesDir, "Codio")
    private val projectsRoot = File(codioRoot, "Projects")

    init {
        listOf("Projects", "Runtime", "OpenCode", "Cache", "Downloads", "Backups")
            .map { File(codioRoot, it) }
            .forEach { it.mkdirs() }
    }
    ...
    fun projectDir(projectId: String): File {
        ...
        if (!directory.canonicalPath.startsWith(projectsRoot.canonicalPath + File.separator)) {
            throw SecurityException("Project path is outside Codio storage")
        }
        return directory
    }
