...
class WebBridge(
    ...
    fun stopRuntime(): String {
        ...
        return try {
            webView.context.startService(
                android.content.Intent(webView.context, CodioOpenCodeService::class.java)
                    .setAction(CodioOpenCodeService.ACTION_STOP)
            )
    ...
    fun getDiagnostics(): String = call {
        JSONObject()
            ...
            .put("storage", files.codioRoot.absolutePath)
    ...
    private fun emit(type: String, payload: JSONObject) {
        ...
        webView.post {
            webView.evaluateJavascript(
                "window.CodioNative && window.CodioNative.onEvent(${JSONObject.quote(event.toString())})",
                null
            )
        }
    }
